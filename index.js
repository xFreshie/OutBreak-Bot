...
client.on("ready", () => {
  ...
  client.user.setActivity(`Being recoded`);
});
...
var wot = [
  ...
  if(command === "roast") {
       if (msg.guild && !msg.channel.permissionsFor(msg.guild.me).has(["SEND_MESSAGES", "VIEW_CHANNEL"])) return;
        ...
        return msg.say(`${user}, you know what? you're nothing but ${start[Math.floor(Math.random() * start.length)]} ${middle[Math.floor(Math.random() * middle.length)]} ${end[Math.floor(Math.random() * end.length)]}.`);
  }
